...
const Navbar = () => {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 w-full z-[100] transition-all duration-300 ${isScrolled ? "bg-white shadow-md" : "bg-white"}`}
      >
        {/* Top right language button */}
        <div
          className={`hidden lg:flex absolute top-0 right-0 p-2 md:p-4 transition-opacity duration-300 ${isScrolled ? "opacity-0 pointer-events-none" : "opacity-100"}`}
        >
        ...
        {/* Main Navbar */}
        <nav
          className={`max-w-[1440px] mx-auto transition-all duration-300 ${isScrolled ? "py-1" : "pt-4 md:pt-6 lg:pt-12 pb-4"}`}
        >
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex-shrink-0 flex items-center cursor-pointer">
              <img
                src="/Assets/logo.png"
                alt="Garibook Logo"
                className="h-14 md:h-14 lg:h-14 w-auto object-contain"
              />
            ...
            <div className="hidden lg:flex items-center space-x-4 lg:space-x-1 font-medium">
              {[
                ...
                <a
                  key={item}
                  href="#"
                  className="relative text-[#121212] text-[20px] px-2 py-1 hover:text-garibook-blue transition-colors group"
                >
                  {item}
            ...
            <div className="lg:hidden flex items-center gap-3">
              ...
              <button
                ...
                className="text-gray-700 hover:text-[#0E52FF] focus:outline-none"
              >
                <svg
                  className="h-7 w-7"
                  fill="none"
        ...
        {/* Mobile Menu Overlay */}
        <div
          className={`fixed inset-0 bg-[#0E52FF] z-[100] transform transition-transform duration-300 flex flex-col ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}
        >
          {/* Top Header of Menu */}
          ...
          <div className="flex-1 flex flex-col items-center pt-8 gap-6 relative z-10">
            {[
              ...
              <a
                key={item}
                href="#"
                className="text-white text-[16px] font-medium hover:text-gray-200 transition-colors"
              >
                {item}
              </a>
            ))}
